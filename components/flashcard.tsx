...
export function Flashcard({
  ...
  const initAudio = useCallback(() => {
    ...
    fetch("/magazine-drop.mp3")
      .then(r => r.arrayBuffer())
      .then(buf => ctx.decodeAudioData(buf))
      .then(decoded => { audioBufferRef.current = decoded })
      .catch(() => {})
    fetch("/correct-incorrect-ring-sound.mp3")
      .then(r => r.arrayBuffer())
      .then(buf => ctx.decodeAudioData(buf))
      .then(decoded => { resultBufferRef.current = decoded })
      .catch(() => {})
  }, [])
  ...
  const triggerShake = useCallback(() => {
    // Play magazine-drop sound for 300ms on impact
  ...
  }, [isFlipped, handleFlip, handleResult, initAudio])
  ...
  return (
    <div 
      ...
      style={{
        ...
        transition: shakeOffset.x === 0 && shakeOffset.y === 0 ? "transform 0.18s ease-out" : "transform 0.065s ease-out"
      }}
    >
      ...
      <motion.div
          ...
          animate={
            exitDirection === "right" ? { x: "120%", rotate: 12, opacity: 0, scale: 1, y: 0 }
            : exitDirection === "left"  ? { x: "-120%", rotate: -12, opacity: 0, scale: 1, y: 0 }
        ...
        >
          <div
            ...
            style={{ 
              ...
              transform: isEntering ? "none" : `rotateX(${tilt.x}deg) rotateY(${(isFlipped ? 180 : 0) + tilt.y}deg)`,
            }}
          >
            {/* Front - Question */}
            <div
            ...
            >
              ...
              <div className="absolute top-5 left-6 z-10">
                <div className="flex items-center gap-2">
                  <div className={cn("w-2 h-2 rounded-full", getAccentColor(familiarity))} />
      ...
      <div className={cn(
        ...
        isFlipped && !exitDirection ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2 pointer-events-none"
      )}>
        <button
          onClick={(e) => {
            e.stopPropagation()
